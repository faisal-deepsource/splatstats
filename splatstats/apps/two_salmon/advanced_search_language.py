...
class Lexer:
    ...
    def attr_var_keyword(self):
        ...
        value = "".join(result)
        if regex.search(
            "^((wave_[1-3]_((power_eggs)|(golden_((delivered)|(appear)))|(quota)|(water_level)|(event_type))|(schedule_((weapon_[0-3])|((start)|(end))time)))|(stage)|(((play)|(end)|(start))time)|(grade_point(_delta)?)|(job((_score)|(_failure_reason)|(_id)))|(is_clear)|(failure_wave)|(danger_rate)|(((steel_eel)|(maws)|(scrapper)|(stinger)|(steelhead)|(flyfish)|(drizzler)|(griller)|(goldie))_count)|(((player)|(teammate[0-2]))_((species)|(gender)|(title)|(((golden)|(power))_eggs)|(name)|(special)|(weapon_w[1-3])|(((revive)|(death))_count)|(id)|(((steel_eel)|(maws)|(scrapper)|(stinger)|(steelhead)|(flyfish)|(drizzler)|(griller)|(goldie))_kills)|(w[1-3]_specials))))$",
            value,
        ):
...
class Interpreter:
        ...
        self.vars = [
            {
                ...
                "stages": [x for (x, y) in stages if x != "all"],
                "titles": [x for (x, y) in titles],
                "events": [x for (x, y) in events],
                "fail_reasons": [x for (x, y) in fail_reasons],
                "weapon_class": [x for (x, y) in weaponclass],
                "weapon_specials": [x for (x, y) in specials if x != "all"],
                "tides": [x for (x, y) in tides],
            }
        ]
        self.switch_comp = {
            "gt": lambda a, b: a > b,
    ...
    def interpret(self):
        ...
        return Shift.objects.filter(q_obj)
    ...
    def len_handler(self, evaluate=True):
        ...
        if isinstance(value, dict):
            result = Shift.objects.filter(self.q_dict_to_q(value)).count()
        elif isinstance(value, Q):
            result = Shift.objects.filter(value).count()
    ...
    def comp_op_handler(self, evaluate=True):
        ...
        if isinstance(term_a, Token) and term_a.type is ATTR:
            if regex.search("weapon$", term_a.value):
                value_a = [x for (x, y) in weapons if y == term_b]
                if len(value_a) > 0:
                    term_b = value_a[0]
            elif regex.search("special$", term_a.value):
                value_a = [x for (x, y) in specials if y == term_b]
                if len(value_a) > 0:
                    term_b = value_a[0]
                ...
                value_a = [x for (x, y) in stages if y == term_b]
                if len(value_a) > 0:
                    term_b = value_a[0]
            ...
            if regex.search("^teammate[0-2]_weapon$", list(term_a.values())[0]):
                value_a = [x for (x, y) in weapons if y == term_b]
                result = {}
                if len(value_a) > 0:
                    term_b = value_a[0]
